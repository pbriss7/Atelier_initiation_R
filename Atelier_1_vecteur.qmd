...
À tout moment, sans même sortir de l'environnement de travail, on peut obtenir de l'aide sur une fonction en particulier ou sur une extension (ou "package").

### Documentation sur les extensions

| Pour illustrer le travail avec les extensions, nous utiliserons `proustr`, créé par Colin Fay en 2017. Cette extension contient les sept romans composant *La Recherche du temps perdu* de Marcel Proust, ainsi que la liste de tous les personnages de la *Recherche*. L'extension contient également des fonctions permettant par exemple de choisir des citations au hasard.
...
**Rappel**: vous trouverez toutes les extensions approuvées par l'équipe de RStudio sur [CRAN("Comprehensive R Archive Network")](%22https://cran.r-project.org/%22). Il y en a plus de 21 400 en ce mois d'octobre 2024.
...
# Par convention, on utilise l'opérateur <- pour les assignations et le symbole `=` pour définir les arguments d'une fonction, mais les deux fonctionnent pour l'assignation.
...
Les deux objets que nous avons créés dans la section précédente sont des vecteurs de longueur 1. Il ne faut pas confondre la longueur de la chaîne de caractères, c'est-à-dire le nombre de caractères que contient une chaîne, et la longueur d'un objet, soit le nombre d'éléments dont il est constitué. Le nombre de caractères d'une chaîne peut être obtenu avec la fonction `nchar()`, tandis que la longueur de l'objet est obtenue avec `length()`.
...
| Nom de la fonction | Extension d'origine | Ce que la fonction produit ou renvoie |
|-------------------|-------------------|----------------------------------|
| `list.files()` | base | Un vecteur contenant les noms des éléments d'un dossier |
| `install.packages()` | base | L'installation d'une extension à partir de CRAN |
| `library()` | base | L'activation d'une extension installée |
| `help()` | base | La documentation sur une fonction ou une extension |
| `proust_random()` | proustr | Une phrase tirée de *La Recherche du temps perdu* |
| `nchar()` | base | Le nombre de caractères d'une chaine |
| `length()` | base | Le nombre d'éléments d'un objet. Appliquée à une table, la fonction renverra le nombre de colonnes. |
| `mode()` | base | Le type de structure d'un objet |
| `print()` | base | Le contenu d'un objet |
| `str()` | base | La structure d'un objet |
| `identical()` | base | L'identité de deux objets. Renvoie TRUE si les objets sont identiques et FALSE s'il y a la moindre différence entre les deux objets. |
| `c()` | base | Un vecteur d'une longueur équivalente au nombre d'éléments concaténés, tous du même type. |
| `paste()` | base | Un vecteur de type `character` combinant deux ou plusieurs autres vecteurs. |
