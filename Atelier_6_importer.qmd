...
Prenons maintenant un autre cas de figure, soit un fichier de format `.txt`. Comme il ne s'agit pas de données structurées, on utilisera la fonction `readLines()`. Cette fonction prend comme argument principal le chemin vers le fichier à importer, `con =`.
...
Le fichier qu'on vient d'importer provient du site du Projet Gutenberg. Il se trouve à l'adresse suivante: <https://www.gutenberg.org/ebooks/13525> . Ce même URL pourrait être fourni en entrée à la fonction `readLines()`, en lieu et place du chemin vers un fichier local.
...
scouine$data[[1]]$text # On obtient ici la structure du vecteur de texte


# Cette structure (un vecteur) peut être assignée à une variable
scouine_v <- scouine$data[[1]]$text

View(scouine_v)
```

Si vous avez exécuté la dernière ligne du bloc ci-dessus, vous aurez pu visualiser le texte de la Scouine et constater qu'il y a un peu de "ménage" à faire dans la chaîne de caractères. C'est une chose que vous pourrez faire assez aisément à la fin de la série d'ateliers.

## Trouver le chemin des fichiers

L'importation d'un fichier suppose que vous sachiez exactement où celui-ci se trouve dans la hiérarchie des dossiers. Trouver ce chemin est beaucoup moins compliqué qu'il n'y paraît! Il existe plusieurs manières d'y arriver. On en propose deux ci-dessous.

### 1. Utiliser l'interface graphique.

Dans RStudio, vous pouvez utiliser l'interface graphique. Sous l'onglet `Files`, vous naviguez vers le fichier que vous souhaitez importer. Les deux points `..` situés en haut de la fenêtre des dossiers vous permet, comme la flèche située tout juste à côté de ces deux points, de monter d'un cran dans la hiérarchie des dossiers. Une fois que vous avez repéré le fichier que vous souhaitez importer, vous cochez la case située à sa gauche, cliquez ensuite la flèche descendante de l'onglet `More`, et choisissez l'option "Copy Folder Path to Clip Board".
...
Dans un environnement Windows, on pourra utiliser la commande `Get-ChildItem` dans le PowerShell. Voir [cet article](https://www.tutos.eu/9402) pour plus de détails.

## Résoudre les problèmes d'encodage

Nous allons tenter d'importer à nouveau le tableau de données provenant de Données Québec, mais sans fournir d'argument à `fileEncoding =`.

```{r}
publications <- read.csv(file = "donnees/publicationsqc.csv", 
                         fileEncoding = "",
                         header = TRUE)
