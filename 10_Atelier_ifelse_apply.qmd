...
Nous allons ajouter une colonne à ce tableau indiquant si le film a été réalisé au XXe siècle ou au XXIe siècle. Il n'y aura donc que deux options possibles. Le test, qui forme le premier argument de la fonction ifelse() et qui doit renvoyer une valeur logique (`TRUE` / `FALSE`), peut se présenter comme suit : `realisatrices$ANNEE < 2000`. Le deuxième argument de la fonction est formé de l'instruction en cas de résultat `TRUE` et le troisième argument doit comprendre l'instruction en cas de résultat `FALSE`. Voici l'instruction complète:
...
Cependant, elle sera plus lente, considérablement plus lente, comme le montre ce test effectué avec l'extension `rbenchmark`:
...
Les fonctions de la famille `apply()` permettent d'effectuer les mêmes tâches que les boucles, mais elles sont plus concices et parfois plus rapides. En gros, ces fonctions appliquent une *autre* fonction sur chaque élément d'un objet récursif (un vecteur, une liste, ...). La fonction de base est `apply()` permet d'appliquer une fonction à des lignes ou des colonnes d'une matrice ou d'un tableau de données. Composons une matrice pour illustrer l'utilisation de cette fonction. Dans la première instruction `apply()`, la somme sera calculée sur la base des lignes (argument `MARGIN=1`), dans la deuxième instruction, elle sera calculée sur la base des colonnes (argument `MARGIN=2`). Le résultat de chacune des instructions sera un simple vecteur fournissant la somme des lignes ou des colonnes.
...
Les "saveurs" `sapply()` et `lapply()` de cette famille prennent en entrée un vecteur ou une liste et retournent un vecteur (`sapply()`) ou une liste (`lapply()`) de la même longueur que les objets pris en entrée. Elles appliqueront une fonction à chacun des éléments de la liste ou du vecteur donné. Prenons un exemple très simple pour commencer. Soit le tableau de données `realisatrices` importé au début de l'atelier. Supposons que nous souhaitions savoir de quel type de données chaque colonne du tableau est composé. Nous pourrions insérer la fonction `class()` à l'intérieur de `sapply()` ou de `lapply()`, et celles-ci appliqueraient la fonction `class()` à chacune des colonnes du tableau, puis renverrait le résultat de cette fonction sous forme de vecteur ou de liste.

```{r}
sapply(realisatrices, class) # retourne le résultat dans un vecteur
lapply(realisatrices, class) # retourne le résultat dans une liste
...
fichiers_l <- lapply(nomsFichiers_v, read_xls, 1)

# Visualisons le résultat sous la forme d'un tableau
View(as.data.frame(fichiers_l))
```

La grande différence entre les fonctions de la famille `apply()` et les boucles `for()` et `while()` est cosmétique. Dans une boucle, on mettra entre les accolades {...}, les unes à la suite des autres, toutes les opérations qu'on souhaite appliquer à chacun des éléments d'un objet récursif. Comme les fonctions `apply()`, `lapply()`, `sapply()` ou `tapply()` prennent en entrée une fonction, et non une multitude d'instructions, on pourra créer une fonction qui rassemble toutes ces instructions, puis passer celle-ci à `apply()`, `lapply()`, `sapply()` ou `tapply()`.

Prenons une tâche chronophage et répétitive telle que l'importation conditionnelle des extensions et leur activation. Si j'ai une dizaine d'extensions à installer et à activer au début de mon script, je peux le faire manuellement, en me répétant:
...
for(i in seq_along(extensions_v)) {
  if(!extensions_v[i] %in% rownames(installed.packages())){
    install.packages(extensions_v[i])
  }
  require(extensions_v[i], character.only = TRUE)
}
```

Ou je peux créer une fonction, puis la donner en entrée à `lapply()`:
...
Il y a plus d'un avantage à procéder de cette dernière manière. Tout d'abord, une fonction est un bloc de code exportable et réutilisable. Comme il n'intègre pas les données, ce bloc de code peut être copié et collé dans un autre script, ou encore être intégré à une extension. Les extensions que nous importons dans notre environnement contiennent de tels blocs et c'est pourquoi nous pouvons les réutiliser comme bon nous semble, voire les "ouvrir" et les modifier pour qu'ils s'ajustent à nos besoins. Lorsque cela est possible, on gagne ainsi à séparer les données des méthodes. Lorsqu'on a acquis cette dextérité, les fonctions de la famille `apply()` (notamment `lapply()` et `sapply()`) deviennent nos meilleurs alliées!
